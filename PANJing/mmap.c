...
{
	if(remap_pfn_range(vma, vma->vm_start,virt_to_phys(globalmem_devp->mem)>>PAGE_SHIFT, vma->vm_end - vma->vm_start, vma->vm_page_prot))
		return -EAGAIN;  
	vma->vm_ops = &globalmem_remap_vm_ops;
	globalmem_vma_open(vma);
	
	(globalmem_devp->mem)[0] = 'c';
	return 0;
}
static void globalmem_vma_open(struct vm_area_struct *vma)
{
	printk(KERN_NOTICE "globalmem VMA open, virt %lx, phys %lx\n",vma->vm_start, vma->pgoff << PAGE_SHIFT);
}
